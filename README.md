...
test-dotnet-topic-publisher Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-dotnet-topic-publisher-local` in docker compose files    |
...
`docker-compose -p test-dotnet-topic-publisher-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-dotnet-topic-publisher-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-dotnet-topic-publisher-snd1
    - test-dotnet-topic-publisher-snd2
    - test-dotnet-topic-publisher-snd3
variables:
    - test-dotnet-topic-publisher-APPLICATION-SECRET
```
